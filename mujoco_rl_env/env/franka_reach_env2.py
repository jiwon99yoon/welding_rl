...
#         # 키프레임에서 기본 위치 가져오기
#         try:
#             lap_start_key = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_KEY, "lap_start_pose")
#             lap_end_key = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_KEY, "lap_end_pose")
            
#             if lap_start_key != -1:
#                 self.default_init_qpos = self.model.key_qpos[lap_start_key * self.model.nq:(lap_start_key + 1) * self.model.nq].copy()
#             else:
#                 self.default_init_qpos = np.zeros(self.model.nq)
                
#             if lap_end_key != -1:
#                 self.default_goal_qpos = self.model.key_qpos[lap_end_key * self.model.nq:(lap_end_key + 1) * self.model.nq].copy()
#             else:
#                 self.default_goal_qpos = self.default_init_qpos.copy()
                
#         except:
#             self.default_init_qpos = np.zeros(self.model.nq)
#             self.default_goal_qpos = np.zeros(self.model.nq)
        
#         # 사용자 제공 위치 또는 기본값 사용
#         self.init_qpos = init_qpos if init_qpos is not None else self.default_init_qpos[:self.n_joints]
#         self.goal_qpos = goal_qpos if goal_qpos is not None else self.default_goal_qpos[:self.n_joints]
...
# class FrankaReachEnv(Env):
#     """
#     Franka 로봇이 목표 위치에 도달하는 강화학습 환경
#     Domain Randomization 기능 포함
#     """
#     metadata = {'render_modes': ['human', 'rgb_array'], 'render_fps': 30}
...
#             # Site IDs
#             self.waypoint_sites = [
#                 "lap_start", "lap_waypoint1", "lap_waypoint2", 
#                 "lap_waypoint3", "lap_end"
#             ]
...
#     def _setup_spaces(self):
#         """관찰 및 행동 공간 정의"""
#         # 관찰 공간: [로봇 관절 위치(7), 로봇 관절 속도(7), 목표 관절 위치(7), EE 위치(3), 목표 EE 위치(3)]
#         obs_dim = self.n_joints * 3 + 3 + 3  # 27차원
#         self.observation_space = spaces.Box(
#             low=-np.inf, high=np.inf, shape=(obs_dim,), dtype=np.float32
#         )
...
#         # 목표 End-effector 위치 (목표 관절 위치에서 계산)
#         # 간단히 하기 위해 현재 웨이포인트 위치 사용
#         current_waypoint = self._get_current_waypoint_position()
...
#     def _get_current_waypoint_position(self):
#         """현재 목표 웨이포인트 위치 반환"""
#         # 간단한 예시: lap_end 위치 반환
#         site_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_SITE, "lap_end")
#         if site_id != -1:
...
#         """보상 계산"""
#         # 관절 공간에서의 거리
#         joint_error = np.linalg.norm(self.data.qpos[:self.n_joints] - self.goal_qpos)
        
#         # End-effector와 목표 위치 간 거리
#         ee_site_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_SITE, "ee_site")
#         target_site_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_SITE, "lap_end")
        
#         if ee_site_id != -1 and target_site_id != -1:
#             ee_pos = self.data.site_xpos[ee_site_id]
#             target_pos = self.data.site_xpos[target_site_id]
#             ee_error = np.linalg.norm(ee_pos - target_pos)
#         else:
#             ee_error = joint_error
        
#         # 보상 = 거리 기반 (음수) + 목표 도달 보너스
#         reward = -ee_error - 0.01 * joint_error
        
#         # 목표 도달 시 보너스
#         if ee_error < 0.05:  # 5cm 이내
#             reward += 10.0
...
#             ee_pos = self.data.site_xpos[ee_site_id]
#             target_pos = self.data.site_xpos[target_site_id]
#             info['ee_distance'] = np.linalg.norm(ee_pos - target_pos)
